(() => {
  ...
  // node_modules/@jrc03c/js-text-tools/src/helpers/get-n-gram-set.mjs
  function getNGramSet(raw, maxNGramLength) {
    const out = /* @__PURE__ */ new Set();
    const words = raw.split(/\s/);
    maxNGramLength = maxNGramLength || words.length;
    for (let i = 0; i < words.length; i++) {
      for (let j = 1; j <= maxNGramLength; j++) {
        if (i + j > words.length) {
          break;
        }
        const ngram = words.slice(i, i + j).join(" ").trim();
        if (ngram) {
          out.add(ngram);
        }
      }
    ...
    return fuzzyFind(
      ...
      result.excerpt = [
        ...
        ...result.matches.map((match) => {
          ...
          const left = result.doc.slice(start, index2);
          const middle = "<b>" + result.doc.slice(index2, index2 + match.length) + "</b>";
          const right = result.doc.slice(index2 + match.length, end);
